...
export fn nexa_infer(prompt: [*:0]const u8) [*:0]const u8 {
    ...
    const response = std.fmt.allocPrintZ(allocator, "Nexa (Zig) responde a: {s} - ¡Hola mundo súper rápido!", .{prompt_slice}) catch {
