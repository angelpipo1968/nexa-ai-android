...
/** Inference routing mode for Smart Router */
...
data class NexaUiState(
    ...
    // ─── Smart Routing / Offline AI ───
    val inferenceMode: InferenceMode = InferenceMode.HYBRID,  // Current routing mode
    val isOnDeviceActive: Boolean = false,  // True when using on-device inference
    val hasDownloadedModels: Boolean = false,  // True if any offline model is downloaded
    val npuAvailable: Boolean = false,  // True if device has Snapdragon NPU
    val routingReason: String? = null,  // Last routing decision reason (for UI display)
) {
