...
export async function getWolframAnswer(query: string): Promise<string> {
    ...
    try {
        ...
        const url = `https://api.wolframalpha.com/v1/spoken?i=${encodeURIComponent(query)}&appid=${appId}&language=es`;
        
        const response = await fetch(url);
        
        if (!response.ok) {
            // Si falla la hablada, intentamos la Short Answers API como respaldo
            const shortUrl = `https://api.wolframalpha.com/v1/result?i=${encodeURIComponent(query)}&appid=${appId}`;
