...
// Helper to send a new message in Lark/Feishu
async function sendLarkMessage(token: string, receiveId: string, text: string, receiveIdType: string = 'open_id'): Promise<boolean> {
    const body = {
    ...
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            if (data.code === 0) return true;
            console.warn('[LARK] Lark send message error:', data);
        }
    } catch (err) {
        console.warn('[LARK] Lark send failed, trying Feishu fallback...', err);
    }
    ...
    try {
        ...
        if (res.ok) {
            const data = await res.json();
            if (data.code === 0) return true;
            console.error('[LARK] Feishu send message error:', data);
        }
    } catch (err) {
        console.error('[LARK] Send failed on both domains:', err);
    }
...
export async function POST(req: NextRequest) {
    try {
        ...
        // 3. Custom Bot Menu Click event subscription processing
        if (eventType === 'application.bot.menu_v6') {
            const eventKey = body.event?.event_key;
            const openId = body.event?.operator?.operator_id?.open_id;
            
            if (!eventKey || !openId) {
                return NextResponse.json({ error: 'Formato de menú inválido' }, { status: 400 });
            }

            console.log(`[LARK] Custom Bot Menu clicked: "${eventKey}" by user ${openId}`);
            ...
            let responseMessage = '';
            // Map the defined keys to responses or instructions
            ...
            console.log(`[LARK] Sending menu response to user ${openId}...`);
            const sent = await sendLarkMessage(token, openId, responseMessage);
            
            if (sent) {
                console.log('[LARK] Menu response sent successfully.');
                return NextResponse.json({ status: 'success' });
            } else {
                console.error('[LARK] Failed to send menu response to Lark.');
                return NextResponse.json({ error: 'Failed to send menu response' }, { status: 500 });
            }
        }
